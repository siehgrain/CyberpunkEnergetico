...
namespace FischlWorks_FogWar
{
    ...
    public class csFogWar : MonoBehaviour
    {
        ...
        public void SetSightRange(int fogRevealerIndex, int newSightRange)
        {
            // Verificar se o índice está dentro da faixa
            if (fogRevealerIndex < 0 || fogRevealerIndex >= fogRevealers.Count)
            {
                Debug.LogError("Índice do FogRevealer inválido: " + fogRevealerIndex);
                return;
            }
            FogRevealer fogRevealer = fogRevealers[fogRevealerIndex];

            fogRevealer._SightRange = newSightRange;

            // Atualizar o campo de visão
            shadowcaster.ProcessLevelData(fogRevealer._CurrentLevelCoordinates, Mathf.RoundToInt(fogRevealer._SightRange / unitScale));

            // Forçar atualização da textura da névoa
            ForceUpdateFog();
        }
